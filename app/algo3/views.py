...
def algoKNN_list(request):
    return render(request, 'prueba1/index_algo1.html')
...
def buscar(request):
    if request.GET["k"].isdigit() and request.GET["x1"].isdigit() and request.GET["x2"].isdigit() and request.GET["x3"].isdigit():
        ...
        mensaje = referencia
        return render(request, 'prueba1/resultadoClasificacion.html', {'resultado': referencia})
    else:
...
def muestra_datos(request):
    ...
    return render(request, 'prueba1/index_algo3.html', {'contexto':contexto})
